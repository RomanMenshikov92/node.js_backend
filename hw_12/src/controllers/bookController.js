...
export const getBookPage = async (req, res, next) => {
  try {
    ...
    const updatedBook = await incrementViewCount(req.params.id);
    if (!updatedBook) {
      console.warn("Book not found during view count increment:", req.params.id);
      res.render("books/view", { book, viewCount: book.viewCount, user: req.user });
    } else {
      // Загружаем комментарии из базы
      const comments = await Comment.find({ bookId: req.params.id }).sort({ timestamp: 1 }).lean();
      res.render("books/view", { book, viewCount: updatedBook.viewCount, comments, user: req.user });
    }
  } catch (error) {
